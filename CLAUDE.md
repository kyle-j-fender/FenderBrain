...
## Action Section (`actions.json`)

`actions.json` is this repo's contribution to the cross-repo Chief of
Staff routine (built separately, in `FenderOS`'s
`.claude/skills/chief-of-staff-briefing`), which pulls every household
repo's open near-term actions and places them on Google Calendar
within a rolling 14-day window. It follows the shared schema every
integrating repo uses, with `id_prefix: "brain"`.

Unlike `FenderHouse`/`FenderFinance`/`pm_oraclecmms`/`youngadults.whc`,
this file is **hand-maintained / conversationally populated during a
live Claude Code session — never generated by a standing script that
queries the Second Brain Notion database on its own.** That's a direct
consequence of this file's own "No automation code without a separate
council decision" rule below: a scheduled job or script that reads
Notion to regenerate `actions.json` unattended is exactly the deferred
automation-code option, not something that grows organically out of
adding this file. A live, human-directed session reading Notion's
tools turn by turn (the same model `second-brain/capture.md` documents
for writes) to refresh a few entries is fine and is how this file
should be updated going forward — just like `second-brain/capture.md`'s
capture path, the line is unattended reads, not reads in general.

`calendar_event_id`, `scheduled_on`, and any `status` transition beyond
`open` are owned exclusively by the FenderOS chief-of-staff routine —
don't hand-edit those fields here. This file is a derived export, not
a second system of record: per `second-brain/taxonomy.md`'s "Work
items" section, most of what looks like an actionable item in Second
Brain already belongs (or should belong) to another repo's own system
of record, so `actions.json` here should stay narrow — genuinely
FenderBrain-owned open items (e.g. its own `open-questions.md` gaps),
not a duplicate of what pm_oraclecmms, the Notion Work Tasks Tracker,
FenderHouse, or youngadults.whc already track.
...
**Refresh `actions.json`** — during a live, human-directed session,
optionally do a one-time Notion read of the Second Brain database to
check for new genuinely-unowned open items, then hand-edit the file:
update an existing `open` entry's text/notes if its underlying
`second-brain/open-questions.md` item changed, or append a new entry
(never renumber/reuse an id) if a new item belongs uniquely to
FenderBrain per the Action Section's scoping rule above. Never write a
script that queries Notion to do this automatically — see the Action
Section and this file's automation-code rule.
